...
def get_main_kb(role: EnumUserRoles) -> list[types.BotCommand]:
    if role == EnumUserRoles.GUEST:
        return [
            types.BotCommand(command=EnumCommands.START, description="Перезапустить бота"),
            ...
            types.BotCommand(command=EnumCommands.REGISTER, description="Зарегистрироваться"),
        ]
        ...
        return [
            ...
            types.BotCommand(command=EnumCommands.ASK, description="Спросить у куратора"),
            types.BotCommand(command=EnumCommands.VOTE_PROJECT, description="Проголосовать за проект"),
        ]
